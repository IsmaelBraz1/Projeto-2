...
// Função para calcular a rotação com base na posição do mouse
function onMouseMove(event) {
    ...
    cylinderMesh.rotation.y = mouseX * Math.PI / 4;
    cylinderMesh.rotation.x = mouseY * Math.PI / 4;
    cylinderMesh.rotation.z = mouseX * Math.PI / 4;
}
...
const groundMesh = new THREE.Mesh(groundGeo, groundMat); // Correção aqui
...
window.addEventListener('click', () => {
    ...
    const force = new CANNON.Vec3(launchDirection.x * forceMagnitude*2, launchDirection.y * forceMagnitude*0.9, launchDirection.z * forceMagnitude);
...
const cylinderMaterial = new THREE.MeshBasicMaterial({ color: 0x00ffff, wireframe: false });
const cylinderMesh = new THREE.Mesh(cylinderGeometry, cylinderMaterial);
cylinderMesh.position.set(0, 0, -13);
scene.add(cylinderMesh);
...
// Configuração do Nipple.js (joystick)
...
manager.on('move', function (evt, data) {
    ...
    cube.position.x += sideways * 0.1;
    cube.position.z += forward * 0.1;
});
function animate() {
    world.step(timeStep);
